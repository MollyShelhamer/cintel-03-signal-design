"""
signal_design_shelhamer.py - Project script (example).
...
Terminal command to run this file from the root project folder
    ...
    uv run python -m cintel.signal_design_shelhamer
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_shelhamer.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.7: DEFINE ERROR DENSITY SIGNAL RECIPE
    # ----------------------------------------------------
    # errors per total latency, only when total latency > 0
    is_latency_positive: pl.Expr = pl.col("total_latency_ms") > 0

    calculated_error_density: pl.Expr = pl.col("errors") / pl.col("total_latency_ms")

    error_density_signal_recipe: pl.Expr = (
        pl.when(is_latency_positive)
        .then(calculated_error_density)
        .otherwise(0.0)
        .alias("error_density")
    )
    ...
    df_with_signals: pl.DataFrame = df.with_columns(
        [
            ...
            error_density_signal_recipe,
        ]
    )

    LOG.info(
        "Created signal columns: error_rate, avg_latency_ms, throughput, error_density"
    )
